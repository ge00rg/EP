...
def ep(x, y, var_0=0.000000000001, var_1=1.0, rho=0.1,
       tolerance=0.00000001, maxiter=100, verbose=True):
    '''
    ...
    returns: classifier(see paper equation 46)
    '''
    ...
    # ny[range(n,n+d),range(d)] = p # added by paul
    m = np.zeros((n + d, d))
    ...
    while (not converge and iter_count < maxiter):
        ...
        for i in idx:
            # if(np.any(np.isnan(ny))):
            #     ny[np.isnan(ny)] = np.mean(ny)
            # if(np.any(np.isinf(ny))):
            #     ny[np.isinf(ny)] = np.mean(ny)
            # if(np.any(np.isnan(v))):
            #     v[np.isnan(v)] = np.mean(v)
            # if(np.any(np.isinf(v))):
            #     ny[np.isinf(v)] = np.mean(v)
            #
            # if(np.any(np.isnan(mu))):
            #     v[np.isnan(mu)] = np.mean(mu)
            # if(np.any(np.isinf(v))):
            #     ny[np.isinf(mu)] = np.mean(mu)
            # likelihood terms
            if i < n:
                p_update = False
                # 22-25
                ...
                if (np.any(v_old < 0)):
                    print('[INFO] likelihood Continued, because some v_old < 0.')
                    continue
                # if(np.any(np.isnan(v_old))):
                #     print('[INFO] likelihood Continued, because some v_old = NaN.')
                #     continue
                mu_old = mu + np.multiply(
                    np.multiply(v_old, np.reciprocal(ny[i])), mu - m[i])
                # if(np.any(np.isnan(mu_old))):
                #     print('[INFO] likelihood Continued, because some mu_old = NaN.')
                #     continue
                ...
                #     print('[INFO] likelihood Continued, because some v_new = NaN.')
                #     continue
                # not sure this will be necessary later, but for now...
                if np.any(np.abs(v_new - v_old) <= float("1e-17")):
                    contCounter += 1
                    continue
                ny_new_i = np.reciprocal(
                    np.reciprocal(v_new) - np.reciprocal(v_old))
                # if (np.any(np.isnan(ny_new_i))):
                #     print('[INFO] likelihood Continued, because some ny_new_i = NaN.')
                #     continue
                # if (np.any(np.isinf(ny_new_i))):
                #     print('[INFO] likelihood Continued, because some ny_new_i = NaN.')
                #     continue
                ...
                if (np.any(m_new_i == 0)):
                    print("likelihood m_new_i =0")
                    continue
                s_i = 1
                # if(np.any(np.isinf(mu_new)) or np.any(np.isinf(v_new)) or np.any(np.isinf(ny_new_i)) or np.any(np.isinf(m_new_i))):
                #     idx.remove(i)
                #     continue
                # if(np.any(np.isnan(mu_new)) or np.any(np.isnan(v_new)) or np.any(np.isnan(ny_new_i)) or np.any(np.isnan(m_new_i))):
                #     idx.remove(i)
                #     continue
                # update
                updateCounter += 1
                mu = mu_new
                if (np.any(np.isinf(v_new)) or np.any(np.isnan(v_new))):
                    print("")
                v = v_new
                ny[i] = ny_new_i
                m[i] = m_new_i
                if (np.any(np.isinf(m_new_i))):
                    print("")
                s[i] = s_i
            # prior terms
            else:
                ...
                if np.any(np.abs(v[i - n] - ny[i, i - n]) <= float("1e-17")):
                    contPriorCounter += 1
                    continue
                v_old_i = 1.0 / (1.0 / v[i - n] - 1.0 / ny[i, i - n])
                if (np.any(v_old < 0)):
                    print('[INFO] prior Continued, because some v_old < 0.')
                    continue
                mu_old_i = mu[i - n] + v_old_i * (1.0 / ny[i, i - n]) * (mu[i - n] - m[i, i - n])
                # p_old_i = (p[i - n]/a[i, i - n])/(
                #     p[i - n]/a[i, i - n] + (1.0 - p[i - n])/b[i, i - n])
                p_old_i = rho
                G_0 = norm.pdf(0.0, loc=mu_old_i, scale=np.sqrt(v_old_i + var_0 ** 2))
                G_1 = norm.pdf(0.0, loc=mu_old_i, scale=np.sqrt(v_old_i + var_1 ** 2))
                Z = p_old_i * G_1 + (1 - p_old_i) * G_0
                c_1 = (1.0 / Z) * (p_old_i * G_1 * (-mu_old_i / (v_old_i + var_1 ** 2)) + \
                                   (1.0 - p_old_i) * G_0 * (-mu_old_i / (v_old_i + var_0 ** 2)))
                c_2 = 0.5 / Z * (p_old_i * G_1 * (mu_old_i ** 2 / (v_old_i + var_1 ** 2) ** 2 -
                                                  1.0 / (v_old_i + var_1 ** 2)) + (1 - p_old_i) *
                                 G_0 * (
                                     mu_old_i ** 2 / (v_old_i + var_0 ** 2) ** 2 -
                                     1.0 / (v_old_i + var_0 ** 2)))
                c_3 = c_1 ** 2 - 2.0 * c_2
                ...
                # update
                # if(np.any(np.isinf(mu_new_i)) or np.any(np.isinf(v_new_i)) or np.any(np.isinf(p_new_i)) or np.any(np.isinf(ny_new_nii)) or np.any(np.isinf(m_new_nii)) or np.any(np.isinf(a_new_nii)) or np.any(np.isinf(b_new_nii))):
                #     idx.remove(i)
                #     continue
                # if(np.any(np.isnan(mu_new_i)) or np.any(np.isnan(v_new_i)) or np.any(np.isnan(p_new_i)) or np.any(np.isnan(ny_new_nii)) or np.any(np.isnan(m_new_nii)) or np.any(np.isnan(a_new_nii)) or np.any(np.isnan(b_new_nii))):
                #     idx.remove(i)
                #     continue
                # if (np.any(np.isnan(v_new_i)) or np.any(np.isinf(v_new_i))):
                #     print('[INFO] prior Continued, because some ny_new_i = NaN.')
                #     continue
                # if (np.any(np.isnan(ny_new_nii))):
                #     print('[INFO] prior Continued, because some ny_new_i = NaN.')
                #     continue
                updateCounter += 1
                mu[i - n] = mu_new_i
    ...
    # for now
    print("Result EP", mu, v)
    print("Contcounter = " + str(contCounter))
    print("ContPriorCounter = " + str(contPriorCounter))
    print("There where: " + str(updateCounter) + " parameter updates")
